...
const TableContent = ({
  ...
  useEffect(() => {
    updateSelectedWithIds('couriers', ids, setIds, handleSelected, handleNumSelected);
  }, [ids.couriers]);
...
const Row = ({
  ...
  const handleEditClick = (id) => {
    history.push(`/ui/courier/edit/${id}`);
    handleButtonClick();
  };

  const handleDeleteClick = (id) => {
    const successCallback = (data) => {
      toast.success('Deleted Courier Successfully');
    };

    // handleDeleteRecord(id, API_ENDPOINTS.DELETE_COURIER, fetchRecords, successCallback, (error) => {
    handleDeleteRecord(row.id, API_ENDPOINTS.DELETE_COURIER, fetchRecords, successCallback, (error) => {
      HandleTableErrorCallback(error, 'Courier', ids, setIds); // Pass the error and entity name to the reusable function
    });
  };
