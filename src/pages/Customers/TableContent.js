...
const TableContent = ({
  ...
  useEffect(() => {
    updateSelectedWithIds('customers', ids, setIds, handleSelected, handleNumSelected);
  }, [ids.customers]);
...
const Row = ({
  ...
  const handleEditClick = (id) => {
    history.push(`/ui/customer/edit/${id}`);
    handleButtonClick();
  };
  ...
  const handleDeleteClick = (id) => {
    ...
    // handleDeleteRecord(id, API_ENDPOINTS.DELETE_CUSTOMER, fetchRecords, successCallback, (error) => {
    handleDeleteRecord(row.id, API_ENDPOINTS.DELETE_CUSTOMER, fetchRecords, successCallback, (error) => {
      HandleTableErrorCallback(error, 'Customer', ids, setIds); // Pass the error and entity name to the reusable function
    });
  };
