...
const TableContent = ({
  ...
  useEffect(() => {
    updateSelectedWithIds('cancelled_sale_orders', ids, setIds, handleSelected, handleNumSelected);
  }, [ids.cancelled_sale_orders]);
      ...
      >
        ...
        {columns.filter(column => column.selected).map((column, index) => {
          ...
          if (column.name === 'status') {
            return (
              <td key={index}>
                <div className={`badge h-auto py-0 px-3 ${value === 'Cancelled' ? 'badge-danger' : 'badge-warning'}`}>
                  {value}
                </div>
              </td>
            );
