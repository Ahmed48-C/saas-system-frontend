...
const TableContent = ({
  ...
  useEffect(() => {
    updateSelectedWithIds('clients', ids, setIds, handleSelected, handleNumSelected);
  }, [ids.clients]);
...
const Row = ({
  ...
  const handleEditClick = (id) => {
    history.push(`/ui/client/edit/${id}`);
    handleButtonClick();
  };
  ...
  const handleDeleteClick = (id) => {
    ...
    // handleDeleteRecord(id, API_ENDPOINTS.DELETE_CLIENT, fetchRecords, successCallback, (error) => {
    handleDeleteRecord(row.id, API_ENDPOINTS.DELETE_CLIENT, fetchRecords, successCallback, (error) => {
      HandleTableErrorCallback(error, 'Client', ids, setIds); // Pass the error and entity name to the reusable function
    });
  };
