...
const TableContent = ({
  ...
  useEffect(() => {
    updateSelectedWithIds('balances_history', ids, setIds, handleSelected, handleNumSelected);
  }, [ids.balances_history]);
      ...
      >
        ...
        {columns.filter(column => column.selected).map((column, index) => {
          ...
          if (column.name === 'action') {
            return (
              <td key={index}>
                <div className={`badge h-auto py-0 px-3 ${value === 'DEPOSIT' ? 'badge-success' : 'badge-danger'}`}>
                  {value}
                </div>
              </td>
            );
