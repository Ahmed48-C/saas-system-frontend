...
const TableContent = ({
  ...
  useEffect(() => {
    updateSelectedWithIds('inventory_logs', ids, setIds, handleSelected, handleNumSelected);
  }, [ids.inventory_logs]);
...
const Row = ({
  ...
  const handleDeleteClick = (id) => {
    const successCallback = (data) => {
      toast.success('Deleted Inventory Log Successfully');
    };

    handleDeleteRecord(row.id, API_ENDPOINTS.DELETE_INVENTORY_LOG, fetchRecords, successCallback, (error) => {
      HandleTableErrorCallback(error, 'Inventory Log', ids, setIds); // Pass the error and entity name to the reusable function
    });
  };
